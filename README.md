...
쌓인 로그를 한 번에 훑을 때는 `eval_logs.py`를 씁니다. 파일을 두 개 넘기면 어댑터 교체 전후를 나란히 볼 수 있습니다.

```bash
python eval_logs.py
python eval_logs.py before.jsonl after.jsonl
```
...
| `eval_logs.py` | 수집된 선호 데이터에서 평가 지표 집계 |
